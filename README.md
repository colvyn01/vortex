...
Share this on your network: http://your-computer.local:8000
...
1. Copy the URL from the output (example: `http://your-computer.local:8000`)
...
Then access at: `http://your-computer.local:3000`
...
### Understanding .local Domains

When you start Vortex, you see a URL like `http://your-computer.local:8000`. This is an mDNS (Multicast DNS) address that allows other devices to find your computer by name instead of IP address. Other devices must be on the same WiFi network to access it.

**How .local domains work:**
- Your computer broadcasts its hostname on the local network
- Other devices can resolve `your-computer.local` to find your computer
- No configuration or DNS server required

**mDNS Support by Platform:**
- **macOS/iOS**: Built-in (Bonjour)
- **Linux**: Usually pre-installed (Avahi)
- **Windows 10+**: Built-in, or install [Bonjour Print Services](https://support.apple.com/kb/DL999)

**Localhost (this computer only):**
- Use `--mode localhost` to restrict access to `http://localhost:8000`
...
1. **auto (default)** - Uses hostname.local mDNS format. Falls back to localhost if hostname unavailable.

2. **localhost** - Binds to 127.0.0.1. Only accessible from this computer. Useful for testing.

3. **lan** - Uses hostname.local. Fails if hostname cannot be determined.
...
🔐 Token authentication ENABLED
   Access URL: http://your-computer.local:8000/?token=a1b2c3d4e5f6g7h8
...
In browser: `http://your-computer.local:8000?token=a1b2c3d4e5f6g7h8`
...
Vortex active
Serving directory: /Users/you/Desktop
Share this on your network: https://your-computer.local:8000
...
Chrome/Edge:
1. Click "Advanced"
2. Click "Proceed to your-computer.local (unsafe)"
...
🔐 Token authentication ENABLED
   Access URL: https://your-computer.local:8000/?token=a1b2c3d4e5f6g7h8
   Token: a1b2c3d4e5f6g7h8
