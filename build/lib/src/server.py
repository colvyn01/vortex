...
# File Watcher for Live PDF Updates

_file_watcher: Optional["_FileWatcher"] = None
_file_watcher_lock = threading.Lock()


class _FileWatcher(threading.Thread):
    """
    Daemon thread that monitors a file for changes using os.stat().

    Polls at WATCH_INTERVAL (50ms) and broadcasts file_changed events
    via SSE when the file's modification time changes.
    """

    def __init__(self, file_path: str, session_id: str) -> None:
    ...
    def run(self) -> None:
        """Poll the file for changes at WATCH_INTERVAL."""
        while not self._stop_event.is_set():
            try:
                current_mtime = os.stat(self.file_path).st_mtime
                if current_mtime != self._last_mtime:
                    self._last_mtime = current_mtime
                    self._broadcast_change()
            except OSError:
                pass  # File may be temporarily unavailable during write
            self._stop_event.wait(WATCH_INTERVAL)

    def _broadcast_change(self) -> None:
        """Broadcast file_changed event to all SSE clients."""
        message = {
            "type": "file_changed",
        ...
        _broadcast_message_to_sse(self.session_id, message)
...
def _get_active_devices(session_id: str) -> List[Dict[str, Any]]:
    ...
    with _active_devices_lock:
        ...
        return [
            info
            for info in _active_devices.values()
...
def _cleanup_old_messages_locked(session_id: str) -> None:
    """
    Remove old messages from a chat session.
    NOTE: Must be called with _chat_sessions_lock held.
...
def _add_message(
    ...
    with _chat_sessions_lock:
        ...
        _cleanup_old_messages_locked(session_id)

    # Broadcast to SSE clients (outside lock to avoid holding it during I/O)
...
class VortexHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_api_watch_post(self) -> None:
        """
        Handle POST /api/watch - Start or stop watching a PDF file.

        Request body:
            {"path": "/relative/path/to/file.pdf"} - Start watching
            {"path": ""} - Stop watching

        Response:
            {"status": "watching", "path": "filename.pdf"} or
            {"status": "stopped"}
        """
        ...
        if not file_path.lower().endswith(".pdf"):
            self._send_json({"error": "Only PDF files can be watched"}, 400)
            return
        ...
        # Start new watcher (stop old one first)
        with _file_watcher_lock:
            if _file_watcher:
                _file_watcher.stop()
            _file_watcher = _FileWatcher(full_path, session_id)
            _file_watcher.start()

        self._send_json({"status": "watching", "path": os.path.basename(full_path)})
    ...
    def do_POST(self) -> None:
        ...
        if clean_path == "/api/messages":
            ...
            return
        elif clean_path == "/api/watch":
            self._handle_api_watch_post()
            return
...
def _cleanup_orphaned_uploads(directory: str) -> None:
    """
    Remove orphaned upload temp files from previous server crashes.

    Deletes files starting with '.upload_' that are older than 1 hour.
    This provides a safety margin to avoid deleting active uploads.
    ...
    try:
        for entry in Path(directory).iterdir():
            if entry.name.startswith(".upload_") and entry.is_file():
                try:
...
def run_server(
    ...
    # Clean up orphaned upload temp files from previous crashes
    _cleanup_orphaned_uploads(resolved_directory)
