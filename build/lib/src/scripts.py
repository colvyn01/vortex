...
document.addEventListener('DOMContentLoaded', function() {
	...
	function connectEventSource() {
		...
		eventSource.onmessage = function(event) {
			try {
				...
				// Handle file_changed event - dispatch for PDF viewer
				if (message.type === 'file_changed') {
					document.dispatchEvent(new CustomEvent('vortex-file-changed', {
						detail: message
					}));
					return;
				}
	...
	(function initDirectoryNavigation() {
		...
		// Helper to check if a filename is a PDF file
		...
		document.addEventListener('click', function(e) {
			...
			// Handle PDF files - open in PDF viewer
			if (isPdfFile(filename)) {
				e.preventDefault();
				e.stopPropagation();
				if (window.VortexPdfViewer && typeof window.VortexPdfViewer.open === 'function') {
					window.VortexPdfViewer.open(href, filename);
				}
				return;
			}
